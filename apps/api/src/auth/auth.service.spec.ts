...
describe('AuthService', () => {
  ...
  describe('create_otp', () => {
    ...
    it('should throw BadRequestException for invalid DTO', async () => {
      ...
      await expect(service.create_otp(invalidDto)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('get_user_otps', () => {
    ...
    it('should work with provided session', async () => {
      ...
      const result = await service.get_user_otps(
        'test@example.com',
        entityManager,
      );
  ...
  describe('find_otp_by_otp', () => {
    ...
    it('should throw NotFoundException if OTP not found', async () => {
      ...
      await expect(service.find_otp_by_otp('999999')).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.find_otp_by_otp('999999')).rejects.toThrow(
        "otp doesn't exist",
  ...
  describe('signin_email_otp', () => {
    ...
    it('should sign in successfully with valid OTP', async () => {
      ...
      usersService.update_user_settings_by_user_id = jest
        .fn()
        .mockResolvedValue(mockUserSettings);
      usersService.find_by_id_lock = jest
        .fn()
        .mockResolvedValue(mockUserWithoutPassword);
    ...
    it('should throw BadRequestException for expired OTP', async () => {
      ...
      await expect(service.signin_email_otp(signinOtpDto)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.signin_email_otp(signinOtpDto)).rejects.toThrow(
        'otp expired',
  ...
  describe('sign_in_validated_account', () => {
    ...
    it('should work with provided session', async () => {
      ...
      const result = await service.sign_in_validated_account(
        validUser,
        entityManager,
      );
  ...
  describe('generate_refresh', () => {
    ...
    it('should generate new tokens successfully', async () => {
      ...
      const result = await service.generate_refresh(
        refreshDto.email,
        refreshDto.refresh,
      );
    ...
    it('should throw NotFoundException if refresh token not found', async () => {
      ...
      await expect(
        service.generate_refresh(refreshDto.email, refreshDto.refresh),
