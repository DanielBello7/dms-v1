...
describe('UsersController', () => {
  ...
  describe('update_user', () => {
    ...
    it('should successfully update a user', async () => {
      usersService.modify_user_by_ref = jest
        .fn()
        .mockResolvedValue(mockUpdatedUser);

      const result = await controller.update_user(userId, updateUserDto);

      expect(usersService.modify_user_by_ref).toHaveBeenCalledWith(
        userId,
        updateUserDto,
      );
      expect(usersService.modify_user_by_ref).toHaveBeenCalledTimes(1);
    ...
    it('should throw BadRequestException for invalid DTO', async () => {
      ...
      await expect(controller.update_user(userId, invalidDto)).rejects.toThrow(
        BadRequestException,
      );
      expect(usersService.modify_user_by_ref).toHaveBeenCalledWith(
        userId,
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(
        controller.update_user(userId, updateUserDto),
    ...
    it('should handle partial updates correctly', async () => {
      ...
      usersService.modify_user_by_ref = jest
        .fn()
        .mockResolvedValue(partiallyUpdatedUser);
    ...
    it('should handle updating avatar', async () => {
      ...
      usersService.modify_user_by_ref = jest
        .fn()
        .mockResolvedValue(userWithAvatar);
    ...
    it('should handle updating timezone', async () => {
      ...
      usersService.modify_user_by_ref = jest
        .fn()
        .mockResolvedValue(userWithNewTimezone);
    ...
    it('should ignore restricted fields (password, email, timezone, ref_id) in update', async () => {
      ...
      const result = await controller.update_user(
        userId,
    ...
    it('should propagate errors from service', async () => {
      ...
      usersService.modify_user_by_ref = jest.fn().mockRejectedValue(error);
    ...
    it('should handle empty update object', async () => {
      ...
      usersService.modify_user_by_ref = jest.fn().mockResolvedValue(mockUser);
    ...
    it('should handle multiple field updates', async () => {
      ...
      usersService.modify_user_by_ref = jest
        .fn()
        .mockResolvedValue(multiUpdatedUser);
